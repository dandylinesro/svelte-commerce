...
// 	try {
// 		const data = (await GQL_settings.fetch(event)).data?.settings
// 		settings.set(data)
...
// 		store.set(data)
// 		GQL_storeOne.fetch({ event, variables: { id: storeOne.id } })
// 		const data1 = (await GQL_megamenu.fetch({ event, variables: { store: storeOne.id } })).data
